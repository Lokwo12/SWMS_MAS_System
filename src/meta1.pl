...
chiama_send_message(control_center,pickup_request(Bin,ReqId,Token),_,_,AgM,_,_):-
                send(AgM, send_message(pickup_request(Bin,ReqId,Token), control_center)), !.

chiama_send_message(_,pickup_request(Bin,ReqId,Token),Ag,_,control_center,_,_):-
                atom_concat(truck, _, Ag),
                catch(truck_handle_pickup_request(Ag, Bin, ReqId, Token), E, format('[~w] pickup_error=~q~n', [Ag, E])), !.

chiama_send_message(control_center,assignment(Bin,ReqId,Token),_,_,AgM,_,_):-
                send(AgM, send_message(assignment(Bin,ReqId,Token), control_center)), !.

chiama_send_message(_,assignment(Bin,ReqId,Token),Ag,_,control_center,_,_):-
                atom_concat(truck, _, Ag),
                catch(truck_handle_assignment(Ag, Bin, ReqId, Token), E, format('[~w] assignment_error=~q~n', [Ag, E])), !.

chiama_send_message(_,assignment_ack(_Truck,_Bin,_ReqId,_Token),control_center,_,_,_,_):- !.

chiama_send_message(_,job_accept(Truck,Bin,ReqId,Token),control_center,_,_,_,_):-
                catch(cc_handle_job_accept(Truck, Bin, ReqId, Token), E, format('[control_center] job_accept_error=~q~n', [E])), !.

chiama_send_message(_,job_refuse(Truck,Bin,ReqId,Token),control_center,_,_,_,_):-
                catch(cc_handle_job_refuse(Truck, Bin, ReqId, Token), E, format('[control_center] job_refuse_error=~q~n', [E])), !.

chiama_send_message(_,collection_complete(Bin,ReqId,Token),control_center,_,Truck,_,_):-
                atom_concat(truck, _, Truck),
                catch(cc_handle_collection_complete(Bin, ReqId, Truck, Token), E, format('[control_center] complete_error=~q~n', [E])), !.

chiama_send_message(_,reset_bin(Bin,ReqId,Token),Ag,_,control_center,_,_):-
                atom_concat(smart_bin, _, Ag),
                catch(bin_handle_reset_bin(Ag, Bin, ReqId, Token), E, format('[~w] reset_bin_error=~q~n', [Ag, E])), !.

chiama_send_message(_,reset_ack(Bin,ReqId,Token),control_center,_,_,_,_):-
                catch(cc_handle_reset_ack(Bin, ReqId, Token), E, format('[control_center] reset_ack_error=~q~n', [E])), !.
...
chiama_send_message(_,bin_full(Bin,_),_,_,AgM,_,_):-
                (AgM=smart_bin1;AgM=smart_bin2;AgM=smart_bin3),
                catch(call(handle_bin_full(Bin)), _, true), !.
...
extern(_, pickup_request(_,_,_)).
extern(_, assignment(_,_,_)).
extern(_, job_accept(_,_,_,_)).
extern(_, job_refuse(_,_,_,_)).
extern(_, assignment_ack(_,_,_,_)).
extern(_, collection_complete(_,_,_)).
extern(_, reset_ack(_,_,_)).
...
extern_args(_, pickup_request(_,_,_)).
extern_args(_, assignment(_,_,_)).
extern_args(_, job_accept(_,_,_,_)).
extern_args(_, job_refuse(_,_,_,_)).
extern_args(_, assignment_ack(_,_,_,_)).
extern_args(_, collection_complete(_,_,_)).
extern_args(_, reset_ack(_,_,_)).
