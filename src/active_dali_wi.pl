...
:-dynamic user_profile_location/1,dali_onto_location/1,server_obj/1, specialization/1,own_language/1, agente/4, agent/1, time_charge/1, da_agg/1,
         handled_bin_full/1, request/3, tried/2, truck_state/2, req_seq/1, inflight/6, processed_reply/3, completed_req/1,
         started/0, cycle_counter/1, last_cycle_ts/1,
         truck_local_state/1, truck_current_job/2, cc_watchdog_counter/1.
...
read_server_endpoint(T):-
            open('server.txt',read,Stream,[]),
            read(Stream,T),
            close(Stream).
...
start1(Fe,AgentName,Libr,Fil):-
  ...
  compile(FilePlv),
        ...
        print('..................   Actived Agent '),print(AgentName),print(' ...................'),nl,
        startup_message(AgentName, Message),
        format('~w~n', [Message]),
        ( atom(AgentName), atom_concat(smart_bin, _, AgentName) -> sleep(10), simulate_smartbin_fill(AgentName) ; true ),
        go.

startup_message(control_center, '[control_center] control center online').
startup_message(logger, '[logger] logger online').
startup_message(Name, Message):-
        atom(Name), atom_concat(truck, _, Name),
        atom_concat('[', Name, T1),
        atom_concat(T1, '] Idle, waiting for pickup request', Message).
startup_message(Name, Message):-
        atom(Name), atom_concat(smart_bin, _, Name),
        atom_concat('[', Name, T1),
        atom_concat(T1, '] bin monitor online', Message).
startup_message(Name, Message):-
        atom_concat('[', Name, T1),
        atom_concat(T1, '] online', Message).

smartbin_display_name(Name, Display):-
        atom(Name), atom_concat(smart_bin, Nb, Name),
        atom_concat(smartbin, Nb, Display).
smartbin_display_name(Name, Name).

simulate_smartbin_fill(AgentName):-
        smartbin_display_name(AgentName, DisplayName),
        simulate_smartbin_fill(AgentName, DisplayName, 0).

simulate_smartbin_fill(AgentName, DisplayName, Level):-
        NextLevel is min(Level + 20, 100),
        format('[~w] level = ~w%~n', [DisplayName, NextLevel]),
        catch(send(logger, send_message(log_in(info, level_pct(AgentName, NextLevel), AgentName), AgentName)), _, true),
        ( NextLevel >= 100 ->
            format('[~w] stage=full level = 100% notify=control_center~n', [DisplayName]),
            catch(send(logger, send_message(log_in(warn, bin_full(AgentName), AgentName), AgentName)), _, true),
            wait_for_active_agent(control_center, 10),
            catch(send(control_center, send_message(bin_full(AgentName, city_token_2026), AgentName)), _, true)
        ; sleep(2), simulate_smartbin_fill(AgentName, DisplayName, NextLevel)
        ).

wait_for_active_agent(_, 0).
wait_for_active_agent(AgentName, Attempts):-
        Attempts > 0,
        ( rd_noblock(agente_attivo(AgentName, _)) -> true
        ; sleep(1), Attempts1 is Attempts - 1, wait_for_active_agent(AgentName, Attempts1)
        ).
...
appendi_regole0(Me):-name(Me,L),append(L,[46,116,120,116],Ltf),
                                                                                                         ...
                                                                                                         ( is_communication_base(Me) -> true
                                                                                                         ; append(L,[46,112,108],Ltf1),name(T1,Ltf1),
                                                                                                                 if(file_exists(T1),appendi_regole(T1),true)
                                                                                                         ).

is_communication_base(Me):-atom(Me), sub_atom(Me, _, 13, 0, '/communication').
...
ricmess0(Ag,Ind,AgM,IndM,Language,Ontology,Con):-
          ...
          in_noblock(message(Ind,Ag,IndM,AgM,Language,Ontology,Con)),
            ( Ag = logger, Con = send_message(log_in(Level,Event,Sender), _) ->
                catch(logger_handle_log_in(Level, Event, Sender), _, true)
            ; Ag = logger, Con = send_message(log_event_in(Level,Name,ReqId,Bin,Truck,Note,Sender), _) ->
                catch(logger_handle_log_event_in(Level, Name, ReqId, Bin, Truck, Note, Sender), _, true)
            ; ( Ag = control_center -> catch(cc_maybe_check_timeouts, _, true) ; true ),
              ( Ag = control_center, Con = send_message(bin_full(Bin,_), _) ->
                    catch(cc_handle_bin_full(Bin), E, format('[~w] bin_full_handler_error=~q~n', [Ag, E]))
                ; true
                ),
...
cc_handle_bin_full(Bin):-
        ( handled_bin_full(Bin) -> true
        ; assert(handled_bin_full(Bin)),
          cc_ensure_runtime_state,
            ( request(ReqId, Bin, T), inflight(ReqId, Bin, T, awaiting_assign_ack, _, _) ->
                    Token = city_token_2026,
                ( atom_concat(smart_bin, N, Bin) -> atom_concat(smartbin, N, Bp) ; Bp = Bin ),
                format('[control_center] INFO  retry_assignment  req=~w bin=~w truck=~w~n', [ReqId, Bp, T]),
                    wait_for_active_agent(T, 10),
                send(T, send_message(assignment(Bin, ReqId, Token), control_center))
            ; member(Bin, [smart_bin1, smart_bin2, smart_bin3]), \+ request(_, Bin, _) ->
                ( atom_concat(smart_bin, N, Bin) -> atom_concat(smartbin, N, Bp) ; Bp = Bin ),
                format('[control_center] WARN  bin_full_received  bin=~w~n', [Bp]),
                catch(send(logger, send_message(log_in(warn, bin_full_received(Bp), control_center), control_center)), _, true),
                cc_generate_req_id(ReqId),
                retractall(tried(Bin, _)),
                findall(T, (member(T, [truck1, truck2, truck3]), truck_state(T, idle), \+ tried(Bin, T)), IdleCandidates),
                ( cc_dispatch_pickup(Bin, ReqId, IdleCandidates) -> true
                ; findall(T, (member(T, [truck1, truck2, truck3]), \+ tried(Bin, T)), AnyCandidates),
                  cc_dispatch_pickup(Bin, ReqId, AnyCandidates)
                )
          ; true
          )
        ).
...
cc_ensure_runtime_state:-
        ...
        ( clause(truck_state(_, _), _) -> true ; cc_seed_idle_trucks([truck1, truck2, truck3]) ).

cc_seed_idle_trucks([]).
cc_seed_idle_trucks([T|Rest]):-
        assert(truck_state(T, idle)),
        cc_seed_idle_trucks(Rest).
...
cc_dispatch_pickup(Bin, ReqId, Candidates):-
        Candidates \= [],
        cc_select_truck_random(Candidates, T),
        assert(request(ReqId, Bin, T)),
        retractall(truck_state(T, _)), assert(truck_state(T, busy)),
        Ttl = 6, retractall(inflight(ReqId, Bin, T, _, _, _)),
        assert(inflight(ReqId, Bin, T, awaiting_reply, Ttl, 0)),
        ( atom_concat(smart_bin, N2, Bin) -> atom_concat(smartbin, N2, Bp) ; Bp = Bin ),
        format('[control_center] INFO  pickup_dispatched  req=~w bin=~w truck=~w~n', [ReqId, Bp, T]),
        catch(send(logger, send_message(log_event_in(info, pickup_dispatched, ReqId, Bin, T, '', control_center), control_center)), _, true),
        Token = city_token_2026,
        wait_for_active_agent(T, 10),
        send(T, send_message(pickup_request(Bin, ReqId, Token), control_center)).

%% Random truck selection from available candidates
cc_select_truck_random(Candidates, T):-
        length(Candidates, Len),
        random(X),
        Idx is truncate(X * Len) + 1,
        nth1(Idx, Candidates, T).

%% Truck lifecycle constants
truck_move_time(2).
truck_collect_time(1).

%% Truck: receives pickup_request from control_center
truck_handle_pickup_request(Ag, Bin, ReqId, Token) :-
        Token = city_token_2026,
        ( atom_concat(smart_bin, N, Bin) -> atom_concat(smartbin, N, Bp) ; Bp = Bin ),
        ( ( \+ clause(truck_local_state(_), _) ) ; truck_local_state(idle) ), !,
        format('[~w] Pickup accepted -> ~w~n', [Ag, Bp]),
        catch(send(logger, send_message(log_event_in(info, pickup_accepted, ReqId, Bin, Ag, '', Ag), Ag)), _, true),
        retractall(truck_local_state(_)), assert(truck_local_state(waiting)),
        retractall(truck_current_job(_, _)), assert(truck_current_job(Bin, ReqId)),
        send(control_center, send_message(job_accept(Ag, Bin, ReqId, Token), Ag)).
truck_handle_pickup_request(Ag, Bin, ReqId, Token) :-
        Token = city_token_2026,
        ( atom_concat(smart_bin, N, Bin) -> atom_concat(smartbin, N, Bp) ; Bp = Bin ),
        format('[~w] Pickup refused -> ~w (busy)~n', [Ag, Bp]),
        catch(send(logger, send_message(log_event_in(warn, pickup_refused, ReqId, Bin, Ag, busy, Ag), Ag)), _, true),
        send(control_center, send_message(job_refuse(Ag, Bin, ReqId, Token), Ag)).

%% Truck: receives assignment from control_center — runs move/collect lifecycle
truck_handle_assignment(Ag, Bin, ReqId, Token) :-
        Token = city_token_2026,
        truck_current_job(Bin, ReqId), !,
        ( atom_concat(smart_bin, N, Bin) -> atom_concat(smartbin, N, Bp) ; Bp = Bin ),
        send(control_center, send_message(assignment_ack(Ag, Bin, ReqId, Token), Ag)),
        truck_move_time(M),
        format('[~w] Moving to ~w~n', [Ag, Bp]),
        retractall(truck_local_state(_)), assert(truck_local_state(moving)),
        sleep(M),
        truck_collect_time(C),
        format('[~w] Collecting waste @ ~w~n', [Ag, Bp]),
        retractall(truck_local_state(_)), assert(truck_local_state(collecting)),
        sleep(C),
        format('[~w] Collection completed, reporting to Control Center~n', [Ag]),
        catch(send(logger, send_message(log_event_in(info, collection_done, ReqId, Bin, Ag, '', Ag), Ag)), _, true),
        retractall(truck_local_state(_)), assert(truck_local_state(idle)),
        retractall(truck_current_job(_, _)),
        send(control_center, send_message(collection_complete(Bin, ReqId, Token), Ag)),
        format('[~w] Idle, waiting for pickup request~n', [Ag]).
truck_handle_assignment(Ag, Bin, ReqId, _Token) :-
        ( atom_concat(smart_bin, N, Bin) -> atom_concat(smartbin, N, Bp) ; Bp = Bin ),
        format('[~w] Assignment mismatch -> ~w req=~w~n', [Ag, Bp, ReqId]).

%% CC: receives job_accept from truck — sends assignment
cc_handle_job_accept(Truck, Bin, ReqId, Token) :-
        Token = city_token_2026,
        ( atom_concat(smart_bin, N, Bin) -> atom_concat(smartbin, N, Bp) ; Bp = Bin ),
        ( inflight(ReqId, Bin, Truck, awaiting_reply, Ttl, _) ->
            retractall(inflight(ReqId, Bin, Truck, _, _, _)),
            assert(inflight(ReqId, Bin, Truck, awaiting_assign_ack, Ttl, 0)),
            format('[control_center] INFO  job_accepted  req=~w bin=~w truck=~w~n', [ReqId, Bp, Truck]),
            catch(send(logger, send_message(log_event_in(info, job_accepted, ReqId, Bin, Truck, '', control_center), control_center)), _, true),
            wait_for_active_agent(Truck, 10),
            send(Truck, send_message(assignment(Bin, ReqId, Token), control_center))
        ; format('[control_center] WARN  late_accept_ignored  req=~w bin=~w truck=~w~n', [ReqId, Bp, Truck])
        ).

%% CC: receives job_refuse from truck — try another truck
cc_handle_job_refuse(Truck, Bin, ReqId, _Token) :-
        ( atom_concat(smart_bin, N, Bin) -> atom_concat(smartbin, N, Bp) ; Bp = Bin ),
        format('[control_center] WARN  truck_refused  req=~w bin=~w truck=~w~n', [ReqId, Bp, Truck]),
        catch(send(logger, send_message(log_event_in(warn, job_refused, ReqId, Bin, Truck, '', control_center), control_center)), _, true),
        retractall(truck_state(Truck, _)), assert(truck_state(Truck, idle)),
        retractall(inflight(ReqId, Bin, Truck, _, _, _)),
        retractall(request(ReqId, Bin, Truck)),
        ( \+ tried(Bin, Truck) -> assert(tried(Bin, Truck)) ; true ),
        findall(T, (member(T, [truck1, truck2, truck3]), truck_state(T, idle), \+ tried(Bin, T)), IdleCandidates),
        ( IdleCandidates \= [] -> cc_dispatch_pickup(Bin, ReqId, IdleCandidates)
        ; format('[control_center] WARN  no_idle_trucks  req=~w bin=~w~n', [ReqId, Bp])
        ).

%% CC: receives collection_complete from truck — free truck, send reset_bin to the bin
cc_handle_collection_complete(Bin, ReqId, Truck, Token) :-
        Token = city_token_2026,
        ( atom_concat(smart_bin, N, Bin) -> atom_concat(smartbin, N, Bp) ; Bp = Bin ),
        format('[control_center] INFO  collection_complete  req=~w bin=~w truck=~w~n', [ReqId, Bp, Truck]),
        catch(send(logger, send_message(log_event_in(info, collection_complete, ReqId, Bin, Truck, '', control_center), control_center)), _, true),
        retractall(inflight(ReqId, Bin, Truck, _, _, _)),
        retractall(request(ReqId, Bin, Truck)),
        retractall(truck_state(Truck, _)), assert(truck_state(Truck, idle)),
        retractall(handled_bin_full(Bin)),
        format('[control_center] INFO  reset_bin_sent  req=~w bin=~w~n', [ReqId, Bp]),
        wait_for_active_agent(Bin, 5),
        send(Bin, send_message(reset_bin(Bin, ReqId, Token), control_center)).

%% Bin: receives reset_bin from CC — reset level, send reset_ack, restart fill loop
bin_handle_reset_bin(Ag, _Bin, ReqId, Token) :-
        Token = city_token_2026,
        ( atom_concat(smart_bin, Nb, Ag) -> atom_concat(smartbin, Nb, Bp) ; Bp = Ag ),
        format('[~w] stage=reset level = 0% req=~w~n', [Bp, ReqId]),
        catch(send(logger, send_message(log_event_in(info, bin_reset, ReqId, Ag, none, '', Ag), Ag)), _, true),
        send(control_center, send_message(reset_ack(Ag, ReqId, Token), Ag)),
        sleep(5),
        simulate_smartbin_fill(Ag).

%% CC: receives reset_ack from bin — close the request
cc_handle_reset_ack(Bin, ReqId, Token) :-
        Token = city_token_2026,
        ( atom_concat(smart_bin, N, Bin) -> atom_concat(smartbin, N, Bp) ; Bp = Bin ),
        format('[control_center] INFO  request_closed  req=~w bin=~w~n', [ReqId, Bp]),
        catch(send(logger, send_message(log_event_in(info, request_closed, ReqId, Bin, none, '', control_center), control_center)), _, true),
        retractall(tried(Bin, _)).

%% CC inflight timeout watchdog — logs warnings for stale inflight entries
cc_maybe_check_timeouts:-
        ( clause(cc_watchdog_counter(N), _) -> true ; N = 20 ),
        N1 is N - 1,
        retractall(cc_watchdog_counter(_)),
        ( N1 =< 0 ->
            assert(cc_watchdog_counter(20)),
            findall(f(ReqId,Bin,T,Stage,Ttl), inflight(ReqId,Bin,T,Stage,Ttl,_), L),
            cc_scan_inflight(L)
        ;   assert(cc_watchdog_counter(N1))
        ).

cc_scan_inflight([]).
cc_scan_inflight([f(ReqId,Bin,T,Stage,Ttl)|Rest]):-
        ( Ttl =< 2 ->
            ( atom_concat(smart_bin, Nb, Bin) -> atom_concat(smartbin, Nb, Bp) ; Bp = Bin ),
            format('[control_center] WARN  inflight_timeout_pending  req=~w bin=~w truck=~w stage=~w ttl=~w~n',
                   [ReqId, Bp, T, Stage, Ttl]),
            catch(send(logger, send_message(log_event_in(warn, inflight_timeout_pending, ReqId, Bin, T, Stage, control_center), control_center)), _, true)
        ; true
        ),
        cc_scan_inflight(Rest).

%% Logger: handle simple log_in messages
logger_handle_log_in(Level, Event, _Sender):-
        ( Level = info -> Lvl = 'INFO' ; Level = warn -> Lvl = 'WARN' ;
          Level = error -> Lvl = 'ERROR' ; Lvl = Level ),
        ( Event = bin_full(Bin) ->
            ( atom_concat(smart_bin, N, Bin) -> atom_concat(smartbin, N, Bp) ; Bp = Bin ),
            format('[logger] ~w  bin_full          bin=~w~n', [Lvl, Bp])
        ; Event = bin_full_received(Bp) ->
            format('[logger] ~w  bin_full_received  bin=~w~n', [Lvl, Bp])
        ; Event = level_pct(Bin, NL) ->
            ( atom_concat(smart_bin, _N, Bin) -> atom_concat(smartbin, _N, Bp) ; Bp = Bin ),
            format('[logger] ~w  level_update  bin=~w level=~w%~n', [Lvl, Bp, NL])
        ;   format('[logger] ~w  ~w~n', [Lvl, Event])
        ).

%% Logger: handle rich log_event_in messages
logger_handle_log_event_in(Level, Name, ReqId, Bin, Truck, Note, _Sender):-
        ( Level = info -> Lvl = 'INFO' ; Level = warn -> Lvl = 'WARN' ;
          Level = error -> Lvl = 'ERROR' ; Lvl = Level ),
        ( atom_concat(smart_bin, N, Bin) -> atom_concat(smartbin, N, Bp) ; Bp = Bin ),
        ( Truck = none, Note = '' ->
            format('[logger] ~w  ~w  req=~w bin=~w~n', [Lvl, Name, ReqId, Bp])
        ; Truck = none ->
            format('[logger] ~w  ~w  req=~w bin=~w note=~w~n', [Lvl, Name, ReqId, Bp, Note])
        ; Note = '' ->
            format('[logger] ~w  ~w  req=~w bin=~w truck=~w~n', [Lvl, Name, ReqId, Bp, Truck])
        ;   format('[logger] ~w  ~w  req=~w bin=~w truck=~w note=~w~n', [Lvl, Name, ReqId, Bp, Truck, Note])
        ).
...
em_mess0(message(IndTo,To,IndS,S,Lang,Ont,send_message(pickup_request(Bin,ReqId,Token),Sender)),T,Ag):-
          em_mess(message(IndTo,To,IndS,S,Lang,Ont,send_message(pickup_request(Bin,ReqId,Token),Sender))).

em_mess0(message(IndTo,To,IndS,S,Lang,Ont,send_message(assignment(Bin,ReqId,Token),Sender)),T,Ag):-
          em_mess(message(IndTo,To,IndS,S,Lang,Ont,send_message(assignment(Bin,ReqId,Token),Sender))).
...
check_msg(message(IndTo,To,IndS,S,Lang,Ont,send_message(pickup_request(Bin,ReqId,Token),Sender)),T,Ag):-
          out(message(IndTo,To,IndS,S,Lang,Ont,send_message(pickup_request(Bin,ReqId,Token),Sender))),
          ver_az_int(message(IndTo,To,IndS,S,Lang,Ont,send_message(pickup_request(Bin,ReqId,Token),Sender)),T,Ag), !.

check_msg(message(IndTo,To,IndS,S,Lang,Ont,send_message(assignment(Bin,ReqId,Token),Sender)),T,Ag):-
          out(message(IndTo,To,IndS,S,Lang,Ont,send_message(assignment(Bin,ReqId,Token),Sender))),
          ver_az_int(message(IndTo,To,IndS,S,Lang,Ont,send_message(assignment(Bin,ReqId,Token),Sender)),T,Ag), !.
